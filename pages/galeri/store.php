...
if (in_array($file_ext,$allowed_file_types) && ($filesize < 200000))
{
    ...
    // query
    $query = "INSERT INTO `galeri` (`id_galeri`, `path_galeri`, `caption_galeri`, `tautan_galeri`, `id_user`, `created_at`, `updated_at`) VALUES (NULL, '$path_galeri', '$caption_galeri', '$tautan_galeri', '$id_user', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);";

    $hasil = mysqli_query($db, $query);

    // cek keberhasilan pendambahan data
    if ($hasil == true) {
      echo "<script>window.alert('Tambah galeri berhasil'); window.location.href='../galeri'</script></script>";
    } else {
      echo "<script>window.alert('Tambah galeri gagal!'); window.location.href='../galeri'</script></script>";
    }
